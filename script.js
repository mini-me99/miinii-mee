...
function addToCart(productId, name, price, image) {
    // Get quantity from either the shop view or modal view
    let quantityInput = document.getElementById('quantity' + productId);
    if (!quantityInput) {
        quantityInput = document.getElementById('modalQuantity' + productId);
    }
    
    // Get the quantity value and ensure it's a number
    let quantity = 1;
    if (quantityInput) {
        // Convert the input value to a number and ensure it's valid
    ...
    // Create the cart item with the correct quantity
    const cartItem = {
        productId: productId,
        name: name,
        price: price,
